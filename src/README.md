...
```bash
empirica-case$ sam logs -n HelloEfsFunction --stack-name empirica-case --tail
```
...
```bash
empirica-case$ pip install -r tests/requirements.txt --user
# unit test
empirica-case$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
empirica-case$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name empirica-case
```
